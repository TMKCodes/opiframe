import { getList } from "./shoppingActions.js";
...
export const register = (user) => {
	return async (dispatch) => {
		...
		dispatch(loading());
		let response = await fetch("http://localhost:3080/register",request);
		if(!response) {
...
export const login = (user) => {
	return async (dispatch) => {
		...
		dispatch(loading());
		let response = await fetch("http://localhost:3080/login",request);
		if(!response) {
...
export const logout = (token) => {
	return async (dispatch) => {
		...
		dispatch(loading());
		let response = await fetch("http://localhost:3080/logout",request);
		if(!response) {
			dispatch(clearState());
